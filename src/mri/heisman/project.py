...
def group_rates(current: np.ndarray, games: np.ndarray, group: np.ndarray) -> np.ndarray:
...
def ratios(current, games, group, remaining, final) -> np.ndarray:
    ...
    projected = remaining * group_rates(current, games, np.asarray(group))
    ok = (remaining >= 1) & (projected > 0)
    return np.clip((final - current)[ok] / projected[ok], 0.0, RATIO_CAP)


def project(current, games, group, remaining, ratio_draws) -> np.ndarray:
    ...
    """
    rate = group_rates(np.asarray(current, dtype=float), np.asarray(games, dtype=float), np.asarray(group))
